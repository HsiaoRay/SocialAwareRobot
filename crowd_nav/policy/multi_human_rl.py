...
class MultiHumanRL(CADRL):
    ...
    def obstacle_to_human(self, next_human_states):
        return [next_human_state if isinstance(next_human_state, ObservableState) else ObservableState(
            next_human_state.px, next_human_state.py, 0, 0, next_human_state.radius) for next_human_state in
                             next_human_states]
